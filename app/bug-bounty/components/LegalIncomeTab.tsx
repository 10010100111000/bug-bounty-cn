        ...
        <div className="grid md:grid-cols-2 gap-6">
          {[
            ...
            },
            {
              ...
              icon: <ListChecks className="w-5 h-5 text-[#9FEF00]" />
            },
            {
              title: "了解你所在地区的法律",
              desc: "不同国家和地区的网络犯罪相关法律有很大差异，部分国家哪怕是对白帽黑客，也有严格的法律规定。必须确保你的行为，同时符合你所在地区、以及目标企业所在地区的法律要求。",
              icon: <Scale className="w-5 h-5 text-[#9FEF00]" />
            },
            {
              title: "漏洞修复前绝对不要公开发布",
              desc: "在企业完成漏洞修复、并且明确授权你可以公开之前，绝对不要对外泄露漏洞的任何细节。提前披露不仅会给你带来法律风险，还会严重损害你在行业内的职业声誉。",
              icon: <AlertTriangle className="w-5 h-5 text-[#9FEF00]" />
            }
          ].map((item, index) => (
            <div key={index} className="rounded-lg border border-[#e5e5e5] bg-white p-6 hover:border-[#9FEF00]/30 transition-colors h-full">
          ...
          {[
            ...
            <div key={index} className={`rounded-lg border ${item.color} p-6 h-full flex flex-col`}>
              <div className="flex justify-between items-start mb-2">
                ...
                <span className="text-xs bg-white px-2 py-0.5 rounded-full font-medium">{item.duration}</span>
              </div>
              <div className="mb-4">
                <p className="text-sm text-[#666] mb-1">预期月收入</p>
                <p className="font-bold text-lg">{item.monthlyIncome}</p>
              </div>
              <div className="mb-4">
                <p className="text-sm text-[#666] mb-1">单漏洞平均赏金</p>
                <p className="font-medium">{item.avgBounty}</p>
              </div>
              <div className="mb-4">
                <p className="text-sm text-[#666] mb-2">常见核心漏洞</p>
                <div className="flex flex-wrap gap-2">
                  {item.commonVulns.map((vuln, i) => (
                    <span key={i} className="text-xs bg-white px-2 py-0.5 rounded-full">{vuln}</span>
                  ))}
                </div>
              </div>
              <div className="mt-auto pt-4 border-t border-white/50">
                <h4 className="text-sm font-semibold mb-2">核心建议</h4>
                <p className="text-sm text-[#666]">{item.tips}</p>
              </div>
            </div>
          ))}
        </div>
