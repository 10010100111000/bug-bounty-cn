    ...
    <section>
      ...
      <div className="space-y-4">
        {learningPath.map((item) => (
          <div key={item.level} className="bg-slate-900/40 backdrop-blur-md rounded-lg border border-slate-700/50 p-6">
            <div className="flex flex-wrap items-start justify-between gap-4 mb-4">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-full bg-gradient-to-r from-violet-500 to-cyan-500 text-slate-100 flex items-center justify-center font-bold">
                  {item.level}
                </div>
                <div>
            ...
            <div>
              <h5 className="text-xs font-bold text-slate-400 mb-2">核心工具</h5>
              <div className="flex flex-wrap gap-2">
                {item.tools.map((tool, i) => (
                  <span key={i} className="px-3 py-1 bg-slate-800/60 rounded text-sm font-medium">
                    {tool}
                  </span>
                ))}
              </div>
