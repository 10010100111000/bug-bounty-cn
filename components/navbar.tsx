    ...
    <header className="sticky top-0 z-50 w-full bg-slate-950/70 backdrop-blur-md border-b border-slate-800/60 shadow-[0_4px_30px_rgba(0,0,0,0.1)]">
      <div className="max-w-7xl mx-auto px-4 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          {/* 右侧：导航链接 (自适应滚动/隐藏) */}
          <nav className="flex items-center space-x-1 lg:space-x-2 overflow-x-auto no-scrollbar mask-gradient">
            {navLinks.map((link) => {
              ...
              return (
                <Link
                  key={link.href}
                  href={link.href}
                  className={`relative px-3 py-1.5 lg:px-4 lg:py-2 text-xs lg:text-sm font-semibold rounded-full transition-all duration-300 whitespace-nowrap ${
                    isActive
            ...
            {/* 发光行动呼吁按钮 */}
            <div className="hidden md:block pl-4 ml-2 border-l border-slate-800">
               <a
                 href="/roadmap"
